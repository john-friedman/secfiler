...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="cusipNos" select="p:edgarSubmission/p:formData/p:coverPageHeader/p:issuerInfo/p:issuerCusips/p:issuerCusipNumber" />
	...
	<xsl:template name="issuerInfo" >
		<div class="contentwrapper">
			<table role="presentation"  id="scheader">
				...
				<tr>
					<td class="tableClassNoBorderAlignCenter" >
						...
						<br/>
						<xsl:if test="$submissionType = 'SCHEDULE 13D/A'">
				...
				<table border="1">
					<xsl:call-template name="cusips" />
				</table>
				<tr>
					<td class="center">
						<xsl:for-each select="p:edgarSubmission/p:formData/p:coverPageHeader/p:authorizedPersons/p:notificationInfo">
							<div class="information" style="text-align:center; font-weight:bold; color:blue;">
								<xsl:value-of select="string(p:personName)" />
								<br/>
								<xsl:value-of select="string(p:personAddress/p1:street1)" />,
								<xsl:value-of select="string(p:personAddress/p1:street2)" />
								<br/>
								<xsl:value-of select="string(p:personAddress/p1:city)" />,
								<xsl:value-of select="string(p:personAddress/p1:stateOrCountry)" />,
								<xsl:value-of select="string(p:personAddress/p1:zipCode)" />
								<br/>
								<xsl:value-of select="string(p:personPhoneNum)" />
							</div>
						...
						<div class="information" style="text-align:center; font-weight:bold;">
							<hr class="separatorThin"/>
							<xsl:text>(Name, Address and Telephone Number of Person Authorized to
									Receive Notices and Communications)</xsl:text>
						</div>
					</td>
				</tr>
				<tr>
					<td class="center">
						<div class="information" style="text-align:center; font-weight:bold; color:blue;">
							<xsl:value-of select="string(p:edgarSubmission/p:formData/p:coverPageHeader/p:dateOfEvent)" />
						</div>
						<div class="information" style="text-align:center; font-weight:bold;">
							<hr class="separatorThin"/>
			...
			<div class="information" style="text-align:justify">
				...
				<xsl:choose>
					<xsl:when test="p:edgarSubmission/p:formData/p:coverPageHeader/p:previouslyFiledFlag = 'true'">
	...
	<xsl:template name="formData" match="p:edgarSubmission/p:formData">
		<div class="content">
			<div class="form1">
				<xsl:for-each select="p:reportingPersons/p:reportingPersonInfo">
			...
			<div class="form1">
				<xsl:variable name="stateName">
					<xsl:call-template name="stateDescription">
						<xsl:with-param name="stateCode" select="string(p:coverPageHeader/p:issuerInfo/p:address/p1:stateOrCountry)" />
					</xsl:call-template>
				</xsl:variable>

				<xsl:call-template name="item1through7">
	...
	<xsl:template name="cusips" >
		<!-- 1) Pre-filter: keep only non-empty/whitespace CUSIP elements -->
		<xsl:variable name="cusips"
					  select="p:edgarSubmission/p:formData/p:coverPageHeader/p:issuerInfo/p:issuerCusips/p:issuerCusipNumber[string-length(normalize-space(.)) > 0]"/>

		<!-- 2) Group the filtered list into rows of 5 -->
		<xsl:for-each select="$cusips[position() mod 5 = 1]">
			<tr>
				<!-- Start position within the filtered sequence for this row -->
				<xsl:variable name="start" select="position()"/>

				<xsl:for-each select="$cusips[position() >= $start and position() &lt; $start + 5]">
					<td>
						<div class="information" style="text-align:center; font-weight:bold; color:blue;">
							<!-- Trim any incidental spaces just in case -->
	...
	<xsl:include href="Schedule13D_headerInfoEmpty.xsl" />
	<xsl:include href="Schedule13D_reportingDetails_X02.xsl" />
	<xsl:include href="Schedule13D_1through7.xsl" />
	<xsl:include href="Schedule13D_signature.xsl" />
	<xsl:include href="Schedule13D_documents.xsl" />
