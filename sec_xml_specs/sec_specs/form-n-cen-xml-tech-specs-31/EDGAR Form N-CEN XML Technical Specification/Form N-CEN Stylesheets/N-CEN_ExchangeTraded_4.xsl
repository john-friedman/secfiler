...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="authorizedParticipants">
		<table role="presentation">
			<tr>
				<td>
					<i>Instruction.</i>
					The term "authorized participant" means a member or participant of a clearing
					...
					providers that allows the authorized participant to place orders for the purchase
		...
		<table role="presentation">
			<xsl:for-each select="m1:authorizedParticipants/m1:authorizedParticipant">
				...
				<tr>
					...
					<td>
						<div class="fakeBox2">
							<xsl:value-of select="@authorizedParticipantRssdId" />
							<span>
								<xsl:text>&#160;</xsl:text>
							</span>
						</div>
	...
	<xsl:template name="creationUnits">
		...
		<table role="presentation">
			<tr>
				<td class="label">i. The average percentage of that value composed of cash:</td>
				<td>
				<div class="fakeBox3">
					<xsl:call-template name="format_to_dollar_large">
	...
	<xsl:template name="inKinETF">
		<table role="presentation">
			<tr>
				<td>
					Is the Fund an "In-Kind Exchange-Traded Fund" as defined in rule 22e-4 under the Act (17 CFR 270.22e-4)?
				</td>
				<td>
					<xsl:call-template name="yesNoRadio">
